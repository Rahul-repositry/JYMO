...
const ForgotPass = () => {
  ...
  const [hidePhoneInputs, setHidePhoneInputs] = useState(false);
  ...
  useEffect(() => {
    ...
    if (currentStatus !== "Send OTP") {
      setCurrentStatus("Send OTP");
    }
    ...
    try {
      ...
      if (result.status === "success") {
        const response2 = await axios.post(
          `${process.env.REACT_APP_BACKEND_URI}/api/auth/createsession`,
          { phone: formData.recoveryNumber },
          {
            headers: { "Content-Type": "application/json" },
            withCredentials: true,
          }
